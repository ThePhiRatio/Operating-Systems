...
short int PRegs[4] ; //Saves a ton of switch statements.
int R0 ; //GP regs
int R1 ;
int R2 ;
int R3 ;

int RRegs[4] ; // Tired of those switch statements!
int ACC ;
char PSW[2] ;
...
int main(int argc, char *argv[])
 ...
 void Read_File()
  	{ int fp ;
   	  fp = open("test.5", O_RDONLY) ; //always check the return value. 
	 ...
	 /*Now read in rest of program file*/
  	  while (1)
	...
	{int Done = 0 ;
	 ...
	 while (!Done)
	  {for (i = 0; i < 6 ; i++)
	    ...
	    /*Decode instruction to convert opcode from char to int */

	    opcode = ((int) (IR[0])- 48) * 10 ;
	    opcode += ((int) (IR[1])- 48) ;
	    /* Give user a window into what is happening in the VM */
	...
	*/
	...
	/*Simulate Exdecution of Opcodes*/
