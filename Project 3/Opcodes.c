...
extern int Translate_Address(int);
extern void Scrub_Mem(struct PCB *);


int i, j ;
//curr PCB so we can print information eaiser. 
struct PCB *curr;
...
int ExecuteProc(struct PCB *Current)
	{
	 ...
	 printMEM(100);
	 //sleep(1);
	 //setting curr to Current for easier printing.
	 curr = Current;
	 int Done = 0 ;
	 PC = Current->PC;	//program counter, always starts at 0
	 //return(0) ;
	 //allows us to itterate over the current IC.

	 for(int j = Current->IC; j > 0; j--)
	 /* This is implementing the Instruction Cycle */
	 /*First, fetch instruction */
	  {for (i = 0; i < 6 ; i++)
		IR[i] = memory[Translate_Address(PC)][i] ; //instruction fetch
		...
		printf("Max IC: %d Current IC: %d\n",Current->IC,j);
	    printf("*********************************************************") ;
	    printf("\nIn Program Execution Loop: New PC is %d OPCODE IS %d DONE: %d\n\n", PC, opcode,Done) ;
	    /*Now execute instruction and increment PC (unless Branch) */
	     ...
	     {case 0:    OP0(IR) ; PC++; break ;
	      ...
	      case 99:   printf("Process %d is Done!\n",Current->PID); printMEM(100); sleep(1); if(isScrub){Scrub_Mem(Current);} Done = 1 ;/*if process is done, print 1 and return.*/ printf("DONE: %d\n",Done); return(Done);
	...
	*/
	...
	{int VAL ;
	 ...
	 return(VAL) ;
	 printf("Printing Memory after Fetching.\n");
	 printMEM(100);
	 }
	...
	*/
	...
	printf("Printing Memory after Storing.\n");
	printMEM(100);
	}
	...
	{int i, j ;
	 ...
	 printf("PC is %d\n", PC) ;
	 //print process ID, IC of current PCB
	 printf("Process ID: %d\n", curr->PID);
	 printf("IC: %d\n", curr->IC);
	}
	...
	/*Modified all opcodes that were used in fib code that
	used addresses, which are 4-11 and 20-26 inclusive. Needed to
        modify the Opcodes to account for the base address of each process.*/
	...
	{int PREG, Value, Address ;
	 ...
	 PrintIR(IR) ;
	 PREG = ParseOp1Reg(IR) ;
	 Address = Translate_Address(PRegs[PREG]); 
	...
	{int PREG, Value, Address ;
	 ...
	 PrintIR(IR) ;
         Address = Translate_Address(ParseOP1andOP2Imm(IR));
         StoreData(Address, ACC) ;
	 PrintRegs() ;
	 PrintLocation(Address) ;
	 printf("*********************************************\n\n") ;
	...
	void OP9(char *IR)
	 ...
	 PrintIR(IR) ;
         RREG = ParseOp1Reg(IR) ;
         Address = Translate_Address(ParseOp2(IR));
	...
	void OP20(char *IR)
        {int PREG, VAL ;
         ...
         PrintIR(IR) ;
         PREG = ParseOp1Reg(IR) ;
	 VAL  = FetchData(Translate_Address(PRegs[PREG])) ;
         ACC  += VAL ;
         PrintRegs() ;
	 printf("**********************************************************\n\n") ;
        }
	...
	void OP21(char *IR)
        {int Address, PREG, VAL ;
         ...
         PrintIR(IR) ;
         Address = Translate_Address(ParseOp1(IR));
